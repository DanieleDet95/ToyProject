...
class Calendar extends Model
{
  ...
  private function _showDay($cellNumber)
  {
    ...
    $currentDate_carbon = Carbon::parse($this->currentDate);
    //   if ($currentDate_carbon == $data_carbon) {
    //     $blocco =
    //       '<li id="'  . $data . '" class="red ' . ($cellNumber % 7 == 1 ? ' start ' : ($cellNumber % 7 == 0 ? ' end ' : ' ')) .
    //       ($cellContent == null ? 'mask' : '') . '">' . $cellContent . '</li>';
    //   } else {
    //     $blocco =
    //       '<li id="' . $data . '" class="' . ($cellNumber % 7 == 1 ? ' start ' : ($cellNumber % 7 == 0 ? ' end ' : ' ')) .
    //       ($cellContent == null ? 'mask' : '') . '">' . $cellContent . '</li>';
    //   }
    if ($currentDate_carbon == Carbon::Today()) {
      $blocco =
        '<li id="'  . $this->currentDate . '" class="red ' . ($cellNumber % 7 == 1 ? ' start ' : ($cellNumber % 7 == 0 ? ' end ' : ' ')) .
        ($cellContent == null ? 'mask' : '') . '">' . $cellContent . '</li>';
    } else {
      $blocco =
        '<li id="' . $this->currentDate . '" class="' . ($cellNumber % 7 == 1 ? ' start ' : ($cellNumber % 7 == 0 ? ' end ' : ' ')) .
        ($cellContent == null ? 'mask' : '') . '">' . $cellContent . '</li>';
    }
    return $blocco;
    // }
  ...
  /** * calculate number of days in a particular month */ private function _daysInMonth($month = null, $year = null)
  {
